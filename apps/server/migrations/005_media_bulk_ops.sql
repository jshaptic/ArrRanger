...
-- have been allowed since v1.
--
-- SQLite cannot edit a CHECK constraint, so queue_items is rebuilt. The migration runner
-- disables foreign keys around this file so dropping the old table does not cascade-delete
-- queue_events.

CREATE TABLE queue_items_v5 (
  ...
  depends_on_id  INTEGER          REFERENCES queue_items_v5(id) ON DELETE CASCADE,
...
ALTER TABLE queue_items_v5 RENAME TO queue_items;
