...
if [ "${current}" != "${PUID}:${PGID}" ]; then
  echo "[fleetarr] chown ${CONFIG_DIR} ${current} -> ${PUID}:${PGID}"
  chown -R "${PUID}:${PGID}" "${CONFIG_DIR}"
fi

# Storage roots are reported, never chowned. A recursive chown across a media array is
# slow, destructive, and none of Fleetarr's business: the container simply has to run as
...
if [ -n "${FS_ROOTS:-}" ]; then
  echo "${FS_ROOTS}" | tr ':' '\n' | while IFS= read -r root; do
    ...
    if [ ! -d "${root}" ]; then
      echo "[fleetarr] storage root ${root} is NOT MOUNTED - check the volume binding"
      continue
    fi
    ...
    if su-exec "${PUID}:${PGID}" test -w "${root}"; then
      echo "[fleetarr] storage root ${root} owner=${owner} mode=${mode} writable as ${PUID}:${PGID}"
    else
      echo "[fleetarr] storage root ${root} owner=${owner} mode=${mode} NOT WRITABLE as ${PUID}:${PGID}"
      echo "[fleetarr]   fix: match PUID/PGID to the owner above, or give that group write access"
    fi
  done
fi
...
echo "[fleetarr] starting as ${PUID}:${PGID} (TZ=${TZ:-Etc/UTC})"
